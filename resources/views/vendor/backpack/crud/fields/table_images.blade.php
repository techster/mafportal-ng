        ...
        <table class="table table-bordered table-striped m-b-0"
            ...
            <tr ng-repeat="item in items" class="array-row">
                @foreach( $field['columns'] as $prop => $label)
                    <td>
                        @if($prop == "image")
                            <?php $field_name = $field['name'] . $prop; ?>
                            <input type="text"
                                   ng-model="item.{{ $prop }}"
                                   id="{{$field_name}}<% item.key_id %>-filemanager"
                                   @include('crud::inc.field_attributes')
                                   @if(!isset($field['readonly']) || $field['readonly']) readonly @endif
                            >
                            <div class="btn-group" role="group" aria-label="..." style="margin-top: 3px;">
                                <button type="button" data-inputid="{{$field_name}}<% item.key_id %>-filemanager"
                                        class="btn btn-default popup_selector">
                                    <i class="fa fa-cloud-upload"></i> {{ trans('backpack::crud.browse_uploads') }}
                                </button>
                                <button type="button" data-inputid="{{$field_name}}<% item.key_id %>-filemanager"
                    ...
                    <td class="prev_image_in_repeater prev_image_in_repeater<% item.key_id %>"></td>
                    @else
            ...
            window.angularApp.controller('tableController', function ($scope) {
                ...
                $scope.$watch('items', function (a, b) {
                    ...
                    angular.element(document).ready(function () {
                        $scope.items.forEach(function (name) {
                            if (name.image) {
                                $('.prev_image_in_repeater' + name.key_id).html('<img src="' + name.image + '" width="150" height="70" style="object-fit: cover; object-position: 0 30%;"/>');
                            }
                        });
                    });
...
@push('crud_fields_scripts')
    <script>
        $(document).on('click', '.array-container .popup_selector', function (event) {
            event.preventDefault();

            $.colorbox({
                href: "{{ url(config('elfinder.route.prefix').'/popup/') }}" + "/" + $(this).data('inputid'),
                fastIframe: true,
                iframe: true,
                width: '80%',
                height: '80%'
        ...
        function processSelectedFile(filePath, requestingField) {
            ...
            $('#' + requestingField).change();

            var filePath = "/" + filePath.replace("\\", "/");
            $('#' + requestingField).parent().next('.prev_image_in_repeater').html('<img src="' + filePath + '" width="150" height="70" style="object-fit: cover; object-position: 0 30%; />');
        }

        $(document).on('click', '.array-container .clear_elfinder_picker', function (event) {
            event.preventDefault();
