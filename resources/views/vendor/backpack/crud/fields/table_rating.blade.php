            ...
            <tr ng-repeat="item in items" class="array-row">
                ...
                @foreach( $field['columns'] as $prop => $label)
                    <td>
                        ...
                        @elseif(strpos($prop, "condition1") !== false)
                            <select class="form-control input-sm" ng-model="item.{{ $prop }}" style="min-width: 7em;">
                                <option value="">Any</option>
                                <option value="red">Red (Citizen)</option>
                                <option value="black">Black (Maf)</option>
                                <option value="citizen">Citizen</option>
                                <option value="sheriff">Sheriff</option>
                                <option value="mafia">Mafia</option>
                                <option value="don">Don</option>
                            </select>
                        @elseif(strpos($prop, "condition2") !== false)
                            <select class="form-control input-sm" ng-model="item.{{ $prop }}" style="min-width: 7em;">
                                <option value="">Any</option>
                                <option value="win">Win</option>
                                <option value="fail">Fail</option>
                            </select>
                        @elseif(strpos($prop, "type") !== false)
                            <select class="form-control input-sm" ng-model="item.{{ $prop }}" style="min-width: 7em;">
                ...
                $scope.$watch('items', function (a, b) {
                    ...
                    if (typeof $scope.items != 'undefined') {
                        ...
                        if (typeof $scope.field != 'undefined') {
                            ...
                            if ($scope.items.length == 0) $scope.field.val("");
                        }
                    }
                }, true);
