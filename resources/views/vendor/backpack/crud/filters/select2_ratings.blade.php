...
<li filter-name="{{ $filter->name }}"
    ...
    <a href="#" class="date_style dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">{{ $filter->label }} <span class="caret"></span></a>
    <div class="dropdown-menu">
        <div class="form-group backpack-filter m-b-0">
            <select id="filter_{{ $filter->name }}" name="filter_{{ $filter->name }}" class=" end_date form-control input-sm select2" placeholder="{{ $filter->placeholder }}">
                <option></option>
    ...
    <script>
        jQuery(document).ready(function($) {
            ...
            $(".end_date > option").each(function() {
                if (this.value > today) {
                    $(this).addClass('actualDate_class');
                    var text = $(this).text();
                    $(this).text(text.replace(text, text+' (active)'));
                }
            });

            if($('.end_date option:selected').val() > today) {
                $('.date_style').css({"color":"red", "font-weight":"bold", "font-size":"16px"});
            };
