...
<section class="Gallery Tab RatingCL m-t-100 m-b-130" ng-controller="rating2Ctrl" ng-cloak>
    <div class="container" ng-controller="gallery_archiveCtrl">

        <!--[ BTN ]-->
        <div class="btnWr">
            <a ng-class="{'act': visible}" href="" ng-click="photo()">General</a>
            <a ng-class="{'act': !visible}" href="" ng-click="video()">Main </a>
        </div>
        ...
        <div class="TablWr">
            <div class="toolbar">
                ...
                <div class="filtersWr">
                    <!--[ FORM ]-->
                    <form class="search_form" action="{{ route('home') }}" method="GET">
                        <input class="serchBox"
                               type="text"
                               placeholder="{{trans('clubs.search')}}"
                               onfocus="placeholder='';"
                               onblur="placeholder='Search';"
                               ng-model="query">

                        <input class="serchBtn"
                               type="submit"
                               disabled="disabled"
                               value="">
                    </form>
            ...
            <div class="Rating_table" ng-show="visible">
                <div class="table_content">
                    <table class="body" cellpadding="0" cellspacing="0" border="0">
                        ...
                        <tr>
                            <th>#</th>
                            <th><span>Player's Full Name</span></th>
                            <th><span>Nickname</span></th>
                            <th>
                                <span>
                                    Total Played Games (Total Won)
                                </span>

                                <svg  width="1em" height="1em" viewBox="0 0 16 16" for="svg"
                                ...
                                <a href="javascript:void(0)" onclick="clubRatingClick()" id="svg" class="tooltip" title="Click here to reveal all roles">pellentesque</a>
                            </th>
                            <th class="thead" style="display: none"><span>Citizen (Win)</span>
                            </th>
                            <th class="thead" style="display: none"><span>Mafia (Win)</span>
                            </th>
                            <th class="thead" style="display: none"><span>Sheriff (Win)</span>
                            </th>
                        ...
                        <tr class="animate_repeat"
                            ...
                            <th class="thead" style="text-align: center; display: none">@{{user.Don}} (@{{user.Don_Win}})</th>
                            <th>@{{user.BM}}</th>
                            <th>@{{user.BP}}</th>
                            <th>@{{user.Balls}}</th>
                            <th>@{{ user.PN }}</th>
            ...
            <div class="Rating_table" ng-hide="visible">
                <div class="table_content">
                    <table class="body" cellpadding="0" cellspacing="0" border="0">
                        <tbody>
                        <tr>
                            ...
                            <th>
                                ...
                                <a href="javascript:void(0)" onclick="clubMainRatingClick()"  class="tooltip" title="Click here to reveal all roles">pellentesque</a>
                            </th>
                            <th class="theadMain" style="display: none"><span>Citizen (Win)</span>
                            </th>
                            <th class="theadMain" style="display: none"><span>Mafia (Win)</span>
                            </th>
                            <th class="theadMain" style="display: none"><span>Sheriff (Win)</span>
                            </th>
                            <th class="theadMain" style="display: none"><span>Don (Win)</span></th>
                            <th><span>BM</span>
                            </th>
                            <th><span>BP</span>
                            </th>
                        ...
                        <tr class="animate_repeat"
                            ng-repeat="user in filterFun = (users | filter:query | orderBy:sortType:sortReverse)"
                            ng-if="user.Game >= {{$eligibel_value}}">
                            <th><span>@{{$index + 1}}</span></th>
                            <th>@{{user.Player}}</th>
                            <th>@{{user.Nickname}}</th>
                            <th>@{{user.Game}} (@{{(user.WR - user.Sheriff_Win) + (user.WB - user.Don_Win) +
                            ...
                            <th class="theadMain" style="text-align: center; display: none">@{{user.Citizen }} (@{{user.WR - user.Sheriff_Win}})</th>
                            <th class="theadMain" style="text-align: center; display: none">@{{user.Mafia}} (@{{user.WB - user.Don_Win}})</th>
                            <th class="theadMain" style="text-align: center; display: none">@{{user.Sheriff}} (@{{user.Sheriff_Win}})</th>
                            <th class="theadMain" style="text-align: center; display: none">@{{user.Don}} (@{{user.Don_Win}})</th>
                            <th>@{{user.BM}}</th>
                            <th>@{{user.BP}}</th>
                            <th>@{{user.Balls}}</th>
                            <th>@{{ user.PN }}</th>
...
<script>
    function clubRatingClick() {
        $(".thead").toggle()
    }

    function clubMainRatingClick() {
        $(".theadMain").toggle()
    }
</script>
