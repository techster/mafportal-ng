...
export default async function GalleryPage({ locale = "" }: { locale?: string }) {
  ...
  return <main className="GalleryPage"><div className="container">
    ...
    {unavailable ? <div className="empty">The Python API is unavailable. Start the backend on port 8011.</div> : <>
      <div className="gallery-heading">{labels.photos}</div><section className="GalleryList">{publishedPhotos.map((gallery) => { const preview = gallery.preview?.startsWith("galleries/") ? gallery.preview : gallery.photos[0] ?? gallery.preview; return <a className="gallery-card" href={`${prefix}/gallery/photo/${gallery.slug}`} key={gallery.id}><div className="gallery-card-pic"><MediaImage src={assetUrl(preview)} fallback="/build/img/not_img.jpg" alt={gallery.title} /></div><div className="gallery-card-title">{gallery.title}</div></a>; })}</section>
